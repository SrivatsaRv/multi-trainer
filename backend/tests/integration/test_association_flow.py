...
def test_gym_trainer_invite_flow(client: TestClient, session: Session):
    ...
    r = client.post("/api/v1/auth/login/access-token", data=login_data)
    assert r.status_code == 200
    ...
    r = client.post("/api/v1/auth/login/access-token", data=trainer_login)
    assert r.status_code == 200
    trainer_token = r.json()["access_token"]
    trainer_headers = {"Authorization": f"Bearer {trainer_token}"}
    
    # 3. Setup Gym Profile (Gym Admin)
    gym_payload = {
        "name": "Association Test Gym",
        "slug": "assoc-gym",
        "location": "Test City"
    }
    r = client.post("/api/v1/gyms/", json=gym_payload, headers=gym_headers)
    assert r.status_code == 201
    gym_id = r.json()["id"]
    
    # 4. Setup Trainer Profile (Trainer)
    trainer_payload = {
        "bio": "Ready for association."
    }
    r = client.post("/api/v1/trainers/", json=trainer_payload, headers=trainer_headers)
    assert r.status_code == 201
    ...
    r = client.post(f"/api/v1/gyms/{gym_id}/trainers", json=invite_payload, headers=gym_headers)
    ...
    assert trainers_list[0]["trainer"]["user"]["email"] == "int_trainer_assoc@example.com"
...
def test_trainer_apply_flow(client: TestClient, session: Session):
    ...
    r = client.post("/api/v1/auth/login/access-token", data={"username": gym_email, "password": gym_password})
    gym_token = r.json()["access_token"]
    gym_headers = {"Authorization": f"Bearer {gym_token}"}
    
    # Create Gym
    r = client.post("/api/v1/gyms/", json={"name": "Target Gym", "slug": "target-gym", "location": "City"}, headers=gym_headers)
    gym_id = r.json()["id"]
    ...
    r = client.post("/api/v1/auth/login/access-token", data={"username": tr_email, "password": tr_password})
    tr_token = r.json()["access_token"]
    tr_headers = {"Authorization": f"Bearer {tr_token}"}
    
    # Setup Trainer Profile
    client.post("/api/v1/trainers/", json={"bio": "Applying..."}, headers=tr_headers)
    tr_user_data = client.get("/api/v1/users/me", headers=tr_headers).json()
    trainer_id = tr_user_data["trainer"]["id"]
    ...
    r = client.post(f"/api/v1/trainers/{trainer_id}/gyms?gym_id={gym_id}", headers=tr_headers)
    assert r.status_code == 201
