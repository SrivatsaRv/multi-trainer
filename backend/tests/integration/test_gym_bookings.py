...
def test_get_gym_bookings(client: TestClient, session: Session, test_user):
    # 1. Setup Data
    # Gym
    gym = Gym(admin_id=test_user.id, name="Booking Test Gym", slug="booking-test", location="Remote")
    session.add(gym)
    session.commit()
    session.refresh(gym)
    
    # Trainer
    trainer_user = User(email="trainerB@example.com", hashed_password="pw", full_name="Trainer B", role=UserRole.TRAINER)
    ...
    # Client
    client_user = User(email="clientB@example.com", hashed_password="pw", full_name="Client B", role=UserRole.CLIENT)
