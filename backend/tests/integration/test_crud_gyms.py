...
def test_gym_crud_lifecycle(client: TestClient, session: Session):
    ...
    # Login
    r = client.post("/api/v1/auth/login/access-token", data={"username": email, "password": password})
    assert r.status_code == 200
    ...
    gym_payload = {
        ...
        "slug": "crud-gym",
        "location": "Test City",
        "amenities": ["Wifi", "Parking"] # Assuming list is supported or we test robustly
    }
    ...
    assert gym_data["verification_status"] == "DRAFT"
