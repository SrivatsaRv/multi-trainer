...
def test_approve_gym(client, session, test_user):
    ...
    # Login as admin
    login_data = {"username": "admin@example.com", "password": TEST_USER_PASSWORD}
    ...
    response = client.post(
        f"/api/v1/admin/verifications/gym/{gym.id}/approve", headers=headers
    )
...
def test_reject_gym(client, session, test_user):
    ...
    # Reject gym
    response = client.post(
        f"/api/v1/admin/verifications/gym/{gym.id}/reject", headers=headers
    )
...
def test_approve_trainer(client, session):
    ...
    # Approve trainer
    response = client.post(
        f"/api/v1/admin/verifications/trainer/{trainer.id}/approve", headers=headers
    )
