...
def test_patch_me_endpoint(client: TestClient, session: Session):
    ...
    response = client.patch(
        "/api/v1/users/me",
        json={"full_name": "New Premium Name"}
    )
...
def test_read_trainer_client_robustness(client: TestClient, session: Session):
    ...
    assoc = ClientTrainer(client_id=client_user.id, trainer_id=trainer.id, status=AssociationStatus.ACTIVE, created_at=datetime.now())
    session.add(assoc)
