...
def test_gym_full_crud(auth_client: TestClient, session: Session, test_user: User):
    ...
    if not gym:
        gym = Gym(name="Initial Gym", slug="initial-gym", location="Loc", admin_id=test_user.id)
        session.add(gym)
        session.commit()
        session.refresh(gym)
...
def test_gym_package_crud(auth_client: TestClient, session: Session, test_user: User):
    ...
    response = auth_client.put(f"/api/v1/gyms/{gym.id}/packages/{pkg_id}", json=update_payload)
    assert response.status_code == 200
...
def test_gym_trainer_invitation_flow(auth_client: TestClient, session: Session, test_user: User, trainer_user: User):
    # Setup: Create a gym
    gym = Gym(name="Invite Gym", slug="invite-gym", location="Loc", admin_id=test_user.id)
    session.add(gym)
    
    # Ensure trainer_user has a trainer profile
    trainer_profile = session.exec(select(Trainer).where(Trainer.user_id == trainer_user.id)).first()
    ...
    response = auth_client.patch(f"/api/v1/gyms/{gym.id}/trainers/{trainer_profile.id}/status", json=status_payload)
    assert response.status_code == 200
