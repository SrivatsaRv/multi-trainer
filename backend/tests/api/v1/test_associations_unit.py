...
def test_create_gym_trainer_association(session: Session):
    # Setup
    user_gym = User(
        email="gym_unit@test.com",
        hashed_password="pw",
        full_name="Gym",
        role="GYM_ADMIN",
    )
    user_tr = User(
        email="tr_unit@test.com", hashed_password="pw", full_name="Tr", role="TRAINER"
    )
    ...
    # Test Association
    link = GymTrainer(
        gym_id=gym.id, trainer_id=trainer.id, status=AssociationStatus.INVITED
    )
    session.add(link)
    session.commit()
...
def test_update_association_status(session: Session):
    ...
    user_gym = User(
        email="gym_up@test.com", hashed_password="pw", full_name="Gym", role="GYM_ADMIN"
    )
    user_tr = User(
        email="tr_up@test.com", hashed_password="pw", full_name="Tr", role="TRAINER"
    )
    session.add(user_gym)
    session.add(user_tr)
    session.commit()
    ...
    link = GymTrainer(
        gym_id=gym.id, trainer_id=trainer.id, status=AssociationStatus.PENDING
    )
    session.add(link)
    session.commit()
