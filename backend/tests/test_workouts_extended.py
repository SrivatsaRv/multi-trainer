...
from app.models.workout import (Exercise, ExerciseType, MeasurementUnit,
                                WorkoutTemplate)
from app.models.workout_log import ExerciseLog, WorkoutLog
...
def test_workout_template_extended_crud(
    auth_client: TestClient, session: Session, trainer_context
):
    ...
    response = auth_client.post(
        "/api/v1/workouts/templates", json=template_payload, headers=headers
    )
    ...
    update_payload = {
        "name": "Back & Biceps",
        "exercises": [{"exercise_id": ex.id, "sets": 5, "reps": 10}],
    }
    response = auth_client.put(
        f"/api/v1/workouts/templates/{tmpl_id}", json=update_payload, headers=headers
    )
    assert response.status_code == 200
    ...
    response = auth_client.delete(
        f"/api/v1/workouts/templates/{tmpl_id}", headers=headers
    )
    assert response.status_code == 200


def test_workout_log_creation(
    auth_client: TestClient, session: Session, trainer_context
):
    headers = trainer_context["headers"]
    ...
    response = auth_client.post(
        "/api/v1/workouts/logs", json=log_payload, headers=headers
    )
    assert response.status_code == 201
    ...
    response = auth_client.get(
        f"/api/v1/workouts/logs?client_id={client_user.id}", headers=headers
    )
    assert response.status_code == 200
    logs = response.json()
    assert len(logs) >= 1
