...
def test_create_gym_with_defaults():
    gym = Gym(name="Test Gym", slug="test-gym", location="Test Location", admin_id=1)

    assert gym.name == "Test Gym"
    assert gym.slug == "test-gym"
    assert gym.location == "Test Location"
    assert gym.admin_id == 1
    assert gym.verification_status == VerificationStatus.PENDING
...
def test_gym_create_model():
    gym_create = GymCreate(name="New Gym", slug="new-gym", location="New Location")
...
def test_gym_update_model():
    gym_update = GymUpdate(name="Updated Gym", description="Updated description")
