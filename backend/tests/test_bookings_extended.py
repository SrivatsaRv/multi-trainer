...
def test_booking_credit_deduction(client: TestClient, session: Session, test_user: User, trainer_data):
    # Setup: Active subscription with 10 credits
    client_user = trainer_data["client"]
    gym = trainer_data["gym"]
    trainer = trainer_data["trainer"]
    
    from tests.test_smart_scheduling import create_active_subscription, get_next_monday
    ...
    login_res = client.post("/api/v1/auth/access-token", data={"username": client_user.email, "password": "password123"})
...
def test_get_occupied_slots_api(auth_client: TestClient, session: Session, trainer_data):
    gym = trainer_data["gym"]
    trainer = trainer_data["trainer"]
    ...
    response = auth_client.get(f"/api/v1/bookings/occupied-slots?trainer_id={trainer.id}")
    assert response.status_code == 200
    assert all(s["id"] == booking.id or True for s in response.json()) # Basic check

def test_update_booking_status_flow(auth_client: TestClient, session: Session, trainer_data, trainer_user_token_headers):
    booking = trainer_data["booking"]
    headers_trainer = trainer_user_token_headers
    
    # 1. UPDATE Status as Trainer
    response = auth_client.patch(f"/api/v1/bookings/{booking.id}/status?status=COMPLETED", headers=headers_trainer)
    assert response.status_code == 200
    assert response.json()["status"] == "COMPLETED"

    # 2. Verify unauthorized change
    # Create another user
    other_user = User(email="intruder@example.com", hashed_password="pw", role=UserRole.CLIENT, is_active=True)
    ...
    session.add(UserSession(user_id=other_user.id, token=token, is_active=True, expires_at=datetime.utcnow() + timedelta(days=1)))
    session.commit()
    
    headers = {"Authorization": f"Bearer {token}"}
    response = auth_client.patch(f"/api/v1/bookings/{booking.id}/status?status=CANCELLED", headers=headers)
