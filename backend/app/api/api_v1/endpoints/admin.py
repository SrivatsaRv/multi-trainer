...
# Super Admin check (Platform Admin) - conceptual repurposing of GYM_ADMIN?
# For this MVP, let's assume a Platform Admin user with is_superuser
# or a specific ROLE based on the UserRole enum in user.py.
...
def list_verifications(
    ...
    pending_gyms = session.exec(
        select(Gym).where(Gym.verification_status == VerificationStatus.PENDING)
    ).all()
    pending_trainers = session.exec(
        select(Trainer).where(Trainer.verification_status == VerificationStatus.PENDING)
    ).all()

    return {"gyms": pending_gyms, "trainers": pending_trainers}
