...
from app.models.workout_log import ExerciseLog, WorkoutLog, WorkoutLogCreate
...
def read_templates(
    ...
    # The default relationship loading might be lazy, but SQLModel/Pydantic
    # response will try to access it. To be safe/efficient we logic:
...
@router.post(
    "/templates", response_model=WorkoutTemplate, status_code=status.HTTP_201_CREATED
)
def create_template(
    ...
    if template_in.exercises:
        for ex_data in template_in.exercises:
            ...
            # For MVP let's assume exercise_id is passed OR we look up by name
            # if provided? The previous JSON model implies detailed object.
...
def create_log(
    ...
    for ex_in in log_in.exercises:
        db_ex = ExerciseLog(
            workout_log_id=db_log.id, exercise_name=ex_in.exercise_name, sets=ex_in.sets
        )
        session.add(db_ex)
